---
title: "Probability: Kobe Bryant and 'Hot Hand'"
author: "John Cruz"
output:
...
> A streak length of 1 means one shot was made, followed by a missed shot. When 0, it means it was a missed shot, so no      streak was started. 
...
```{r calc-streak-kobe}
kobe_streak <- as.data.frame(calc_streak(kobe_basket$shot))
colnames(kobe_streak) <- c('length')
```
...
> The distribution is right-skewed. Kobe's typical streak length was 1. His longest streak was 4 consecutive baskets before missing a shot. 
...
```{r set-seed}
set.seed(2000)                 # make sure to change the seed
```
...
```{r table-sim-basket-45}
table(sim_basket)
```

> To make a shooting % of 45%, hits (H) need to be adjusted with *prob* in the *sample()* function. 
...
```{r sim-streak}
sim_streak <- as.data.frame(calc_streak(sim_basket))
colnames(sim_streak) <- c('length')
```
...
> The distribution of our simulated shooter is right-skewed. The typical streak legnth is 1. The longest streak of baskets is 6. 
    
```{r sim-streak-distribution}
ggplot(data = sim_streak, aes(length)) +
  geom_bar() +
  scale_x_continuous(breaks = c(min(sim_streak$length):max(sim_streak$length)))
```
...
> I would expect the distribution to be similarly right-skewed. The longest streak of baskets may change, but the typical streak will be 1.  
...
> Both distributions of Kobe's and the simulated shooter are similarly right-skewed. We do not have evidence that Kobe has a hot hand given that in general he either made the shot or missed, while occasionally going on a longer shooting streak similarly to our simulated player. 

```{r boxplot}
kobe_streak$player <- 'kobe'
sim_streak$player <- 'sim'

combine_streaks <- rbind(kobe_streak, sim_streak)

ggplot(combine_streaks, aes(x = length, fill = player)) +
  geom_histogram(bins = 6, position = 'identity', alpha = 0.25) +
  geom_density(aes(y = after_stat(count), color = player), alpha = 0) +
